...
const Login: React.FC = () => {
  ...
  useEffect(() => {
    if (isAuthenticated) {
      ...
      window.history.pushState(null, "", "/dashboard"); // Trap history
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Replace login page with dashboard and trap history
      navigate("/dashboard", { replace: true });
      window.history.pushState(null, "", "/dashboard"); // Ensure no back navigation
    } catch (err: any) {
      ...
      <div className="flex w-full max-w-5xl h-[500px] shadow-2xl rounded-2xl overflow-hidden">
        ...
        <div className="w-1/2 bg-white p-8 flex flex-col items-center">
          <div className="flex items-center gap-2 mb-4">
            <img className="w-[150px] h-[150px]" src={hsm} alt="hsm" />
          </div>
        ...
        <div className="w-1/2 bg-[#1B4965] p-8 flex flex-col items-center justify-center text-white">
          <img className="w-[70px] h-[70px]" src={hsm2} alt="hsm" />
