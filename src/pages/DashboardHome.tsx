...
//   overdue: OverdueBorrower[];
// };

// const DashboardHome: React.FC = () => {
//   const { dashboard, overdue } = useLoaderData() as LoaderData;
...
const DashboardHome: React.FC = () => {
  ...
  const borrowedPercentage = dashboard.total_borrowed_books + dashboard.total_returned_books > 0 
    ? (dashboard.total_borrowed_books / (dashboard.total_borrowed_books + dashboard.total_returned_books)) * 360
    : 0;

  const displayedBorrowers = overdue.slice(0, 5);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white rounded-lg shadow p-6 flex items-center">
          ...
          <div>
            <h2 className="text-3xl font-bold">
              {dashboard.total_student_count.toString().padStart(4, '0')}
            </h2>
            <p className="text-gray-500">Total User Base</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow p-6 flex items-center">
          ...
          <div>
            <h2 className="text-3xl font-bold">
              {dashboard.total_book_count.toString().padStart(4, '0')}
            </h2>
            <p className="text-gray-500">Total Book Count</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow p-6 flex items-center">
          ...
          <div>
            <h2 className="text-3xl font-bold">
              {dashboard.total_transaction_count.toString().padStart(4, '0')}
            </h2>
            <p className="text-gray-500">Transaction Count</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="mt-4 flex items-center justify-center">
            <div className="flex items-center">
              ...
              <div>
                <div className="flex items-center mb-2">
                  <div className="w-3 h-3 rounded-full bg-[#3D85B0] mr-2"></div>
                  <span>Total Borrowed Books: {dashboard.total_borrowed_books}</span>
                </div>
                <div className="flex items-center">
                  <div className="w-3 h-3 rounded-full bg-[#5DA1CB] mr-2"></div>
                  <span>Total Returned Books: {dashboard.total_returned_books}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="bg-white rounded-lg shadow p-6">
          <h3 className="text-lg font-semibold mb-4">
            Overdue Borrowers ({overdue.length})
          </h3>
          ...
          {overdue.length === 0 ? (
            ...
            <div className="space-y-2">
              ...
              {overdue.length > 5 && (
                <div className="text-center mt-4">
                  <span className="text-sm text-gray-500">
                    Showing 5 of {overdue.length} overdue borrowers
                  </span>
                </div>
              )}
            </div>
